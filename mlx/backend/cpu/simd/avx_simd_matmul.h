...
// ---------------------------------------------------------------------------
// 6 × 16 AVX2 micro‑kernel
// ---------------------------------------------------------------------------
//  C_block  : pointer to C_tile (row‑major, leading dim = ldc, 16‑float stride)
//  A_panel  : pointer to Â (column‑major, stride a_stride = MC_BLOCK)
//  B_panel  : pointer to 𝐁̃ (row‑major,  stride b_stride = NC_BLOCK)
//  kc       : depth (Kc) for this micro‑kernel
//  ldc      : leading dimension of C (in *floats*)
//  a_stride : MC_BLOCK (distance, in floats, between elements (k,i) and (k+1,i))
//  b_stride : NC_BLOCK (distance, in floats, between rows k     and k+1    )
// ---------------------------------------------------------------------------
template <int MR = 6, int NR = 16>
inline void micro_kernel_6x16(
    const float* __restrict A_panel,   // Packed A panel, col-major
    const float* __restrict B_panel,   // Packed B panel, row-major
    float* __restrict C_block,         // Output block in C_acc
    int ldc,                           // Leading dimension of C
    int kc,                            // Number of K iterations
    int a_stride,                      // MC_BLOCK
    int b_stride)                      // NC_BLOCK
