...
template <typename T, int N>
inline Simd<T, N> load_aligned(const T* x);

template <typename T, int N>
inline void store_aligned(T* dst, Simd<T, N> x);
...
// Aligned load for float
template <>
inline Simd<float, 8> load_aligned<float, 8>(const float* x) {
    return Simd<float, 8>(_mm256_load_ps(x)); // Requires 32-byte aligned input
}

// Aligned store for float
template <>
inline void store_aligned<float, 8>(float* dst, Simd<float, 8> x) {
    _mm256_store_ps(dst, x.value); // Requires 32-byte aligned output
}

// Load data of type T and convert to float vector using aligned loads where possible
template <typename T>
inline Simd<float, 8> load_convert_to_float_aligned(const T* src) {
    static_assert(
        std::is_same_v<T, float> || std::is_same_v<T, float16_t> ||
            std::is_same_v<T, bfloat16_t>,
        "load_convert_to_float_aligned requires float, float16_t, or bfloat16_t input");

    if constexpr (std::is_same_v<T, float>) {
        // Use aligned load if input is guaranteed to be aligned
        return load_aligned<float, 8>(src);
    } else if constexpr (std::is_same_v<T, float16_t>) {
#ifdef __F16C__
        // For float16, we need to load 16 bytes (8 half-precision values)
        // Use aligned load if the pointer is guaranteed to be 16-byte aligned
        __m128i f16_vals = _mm_load_si128(reinterpret_cast<const __m128i*>(src)); 
        return Simd<float, 8>(_mm256_cvtph_ps(f16_vals));
#else
        // Scalar fallback with aligned output buffer
        alignas(32) float buffer[8];
        for (int i = 0; i < 8; ++i) buffer[i] = static_cast<float>(src[i]);
        return load_aligned<float, 8>(buffer);
#endif
    } else if constexpr (std::is_same_v<T, bfloat16_t>) {
        // For bfloat16, manual conversion with aligned buffer
        alignas(32) float buffer[8];
...
// Store float vector to aligned memory, converting to destination type T
template <typename T>
inline void store_convert_from_float_aligned(T* dst, Simd<float, 8> src) {
    static_assert(
        std::is_same_v<T, float> || std::is_same_v<T, float16_t> ||
            std::is_same_v<T, bfloat16_t>,
        "store_convert_from_float_aligned requires float, float16_t, or bfloat16_t output");

    if constexpr (std::is_same_v<T, float>) {
        store_aligned<float, 8>(dst, src);
    } else if constexpr (std::is_same_v<T, float16_t>) {
#ifdef __F16C__
        // For float16, convert and store with alignment
        __m128i f16_result = _mm256_cvtps_ph(src.value, _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC);
        _mm_store_si128(reinterpret_cast<__m128i*>(dst), f16_result);
#else
        // Scalar fallback
        alignas(32) float buffer[8];
        store_aligned<float, 8>(buffer, src);
        for(int i=0; i<8; ++i) dst[i] = static_cast<float16_t>(buffer[i]);
#endif
    } else if constexpr (std::is_same_v<T, bfloat16_t>) {
        alignas(32) float buffer[8];
...
// Performs C = alpha * acc + beta * C with aligned operations where possible
template <typename T>
inline void scale_accumulate_store_aligned(
    T* c_ptr,
    Simd<float, 8> acc,
    Simd<float, 8> alpha_vec,
    Simd<float, 8> beta_vec) {
    ...
    if (beta_scalar != 0.0f) {
        // Load existing C values: beta * C[i, j] (use aligned load if possible)
        Simd<float, 8> c_old_vec = load_convert_to_float(c_ptr);
        // result = beta * c_old_vec + result; (using FMA)
        result = fma<float, 8>(beta_vec, c_old_vec, result);
    }

    // Store final result back to C (use aligned store if possible)
    store_convert_from_float<T>(c_ptr, result);
}
